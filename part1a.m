%Part 1a: Doesn't take into account correlated noise

%Top function. Currently, it returns what I think to be the decision
%variable
function[NEURONS D_PRIME] = part1a()
...
function [cov_matrix] = return_cov_matrix(popvec_mean_1, popvec_mean_2, neurons)
    cov_matrix = zeros(neurons, neurons);
    for j=1:neurons
        cov_matrix(j,j) = (popvec_mean_1(j) + popvec_mean_2(j))/2;
    end
end
...
function [w] = weight_calculation(phi1, phi2, c, neurons, K)
    ...
    C = return_cov_matrix(f1, f2, neurons); 
    C_inverse = inv(C);
