...
%Top function.
function[D_PRIME NEURONS] = part1c()
    
    for j=1:300
        K(j) = gamrnd(1/0.6, 0.6);
    end
    
    [D_PRIME offset] = return_d_prime_vs_numberofneurons(pi/3, pi/2, 1, K, 100, 2, 300);
...
%Returns variance matrix to help compute covariance matrix
...
%Calculates d_prime from decision variables across many trials. K is now a
%vector
...
%keeping K constant
function [D_PRIME offset] = return_d_prime_vs_numberofneurons(phi1, phi2, c, K, number_of_trials, neurons_lower, neurons_upper)

    offset=neurons_lower-1;
    
    for i=neurons_lower:neurons_upper        
        D_PRIME(i-offset) = return_d_prime(phi1, phi2, c, i, K, number_of_trials); %Keep K constant at 1
        disp('run no.');
        disp(i-offset);
    end
end
